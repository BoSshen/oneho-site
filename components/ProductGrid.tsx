    ...
    async function fetchProducts() {
      try {
        if (!woocommerce.isConfigured()) {
          setError('WooCommerce is not configured. Please add your WordPress URL and API credentials to the .env file.');
          setLoading(false);
          return;
        }

        const data = await woocommerce.getProducts({ per_page: 12 });
        setProducts(data);
        setError(null);
      } catch (err) {
